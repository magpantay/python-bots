...
# checks if an option ini file value exists and gets that value
def doesExist(search_term, config_contents):
    ret_0 = ""
    ret_1 = 0
    ...
    if config_contents.find(search_term) != -1:
        ...
        if ret_0 != "":
            ...
            print("{0} value present in 'acuitybotconfig.ini' file, but is set to nothing.\n".format(
                search_term.split("='")[0]))
    # ret_0 is the stuff gotten from the search term, ret_1 is the boolean value whether it exists or not
    return [ret_0, ret_1]
...
# prints specific details of parsed JSON text
def printJSON(parsed_json_result, firstTimeRunning=0):
    count = 1  # used for the list numbering
    print("============================================================BEGIN=============================================================\n")
    for each_result in parsed_json_result:
        if firstTimeRunning:
            # taking the endTime (hour) from Acuity and converting to int
            endTimeH = int(each_result['endTime'].split(":")[0])
            # datetime displays in 24H, so convert Acuity's output to 24H
            if each_result['endTime'].split(":")[1].split("m")[0][2] == "p":
                endTimeH = endTimeH + 12
            if clock.now().time().hour > endTimeH:
                continue		# skip since the meeting is over already
            # taking the endTime (minute)
            elif clock.now().time().hour == endTimeH and clock.now().time().minute > int(each_result['endTime'].split(":")[1][0:2]):
                continue
        ...
        print("Name: {0} {1}".format(
            each_result['firstName'], each_result['lastName']))
        print("Phone: {0}".format(each_result['phone']))
        print("Email: {0}".format(each_result['email']))
        print(
            "Appointment Time: {0}-{1}".format(each_result['time'], each_result['endTime']))
        print("Type of Service: {0}".format(each_result['type']))
        print("Work Order/Reference Number: {0}".format(
            each_result['forms'][1]['values'][0]['value']))
        ...
        if each_result['forms'][0]['name'].find("Campus Location") != -1:
            ...
            print("Location: {0} {1}".format(each_result['forms'][0]['values'][0]['value'],
                                             each_result['forms'][0]['values'][1]['value']))  # location + rm/ste value
        ...
        # Bobcat Bar (COB132A)/Non-dorm-run dorm locations#
            ...
            # different notes location for Student Rufus Time location
            print("User explanation of problem: {0}\n".format(
                each_result['forms'][0]['values'][0]['value']))

        # Remote Tech #
        elif each_result['type'] == "Phone/Remote Support":
            # supposedly where the zoom link is
            print("{0}".format(each_result['location']))

        # GP/VT locations for dorm runs #
        elif each_result['forms'][0]['name'] == "Dorm Run Request":
            print("Dorm Location: {0} {1}".format(
                each_result['forms'][0]['values'][2]['value'], each_result['forms'][0]['values'][3]['value']))
            print("What's Broken in the Dorm: {0}".format(
                each_result['forms'][0]['values'][0]['value']))
            print("User explanation of problem: {0}\n".format(
                each_result['forms'][0]['values'][1]['value']))
...
def main():
    ...
    if os_id()[0].find("Linux") != -1 and os_id()[2].find("Microsoft") == -1 or os_id()[0].find("Darwin") != -1:
        print(
    ...
    if not calendarIDisSet or calendarID == "CALENDAR_ID_GOES_HERE":
        ...
        calendarID_list = web_get(
            'https://acuityscheduling.com/api/v1/calendars', auth=(userID, keyAPI))

        if calendarID_list.status_code != 200:  # < 200 OK >
            print(
                "ERROR: Non-OK return status ({0}). Exiting...".format(response))
            exit()
    ...
    params = (('minDate', 'TODAY'), ('maxDate', 'TODAY'),
              ('calendarID', calendarID), )  # for the get request that repeats
    ...
    if pullOnce:
        ...
        response = web_get('https://acuityscheduling.com/api/v1/appointments',
                           params=params, auth=(userID, keyAPI))
        #print("- done.")
        ...
        while 1:
            ...
            response = web_get(
                'https://acuityscheduling.com/api/v1/appointments', params=params, auth=(userID, keyAPI))
            #print ("- done.")
            if response.status_code != 200:  # <200 OK>
                print(
                    "ERROR: Non-OK return status ({0}). Exiting...".format(response))
            ...
            if not first_time_running:
                if something_new:
                    ...
                    print0("Playing beep {0} times".format(
                ...
                # need to account for whether an appointment has been cancelled (but it only plays half the amount of specified beeps)
                    ...
                    printJSON(parsed_json_result)
                    print0("Playing beep {0} times".format(
                        numberOfTimes / 2), numberOfTimes / 2, 1)
                    print("\n================\n")
                else:
                    sleep(0.5)  # just a slight delay
                    print("\nNothing new.")
